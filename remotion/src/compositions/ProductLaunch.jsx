...
function SceneReveal() {
  ...
  return (
    <AbsoluteFill style={{ background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: bgOpacity * fadeOut }}>
      ...
      <GlowOrb top="40%" right="5%" left="auto" size={500} color={B_PURPLE} opacity={0.14} />
      ...
      <div style={{ textAlign: "center", fontFamily: "'Segoe UI', Arial, sans-serif", zIndex: 1 }}>
        ...
        <div style={{ fontSize: 120, fontWeight: 900, color: WHITE, letterSpacing: 4, opacity: nameOpacity, transform: `scale(${nameScale})`, lineHeight: 1 }}>
          MakOne BI
        </div>
        <GradientBar style={{ width: 320, margin: "24px auto 28px" }} />
        <div style={{ opacity: tagOpacity, transform: `translateY(${tagY}px)`, fontSize: 34, color: MUTED, letterSpacing: 2, fontWeight: 300 }}>
...
// ─── Scene 3 — Features (210–480f, ~9s, 3 features × 90f) ────────────────────
const FEATURES = [
  {
    icon: "✍️",
    title: "AI Content Creation",
    desc: "Generate LinkedIn, Facebook & Instagram posts in seconds — powered by GPT-4.",
    color: B_BLUE,
  },
  {
    icon: "🎬",
    title: "Automated Video Generation",
    desc: "HeyGen AI clones, Google VEO 3 cinematic clips & Remotion branded cards — no editing needed.",
    color: B_PURPLE,
  },
  {
    icon: "📅",
    title: "Smart Scheduling & Approval",
    desc: "Review drafts, approve or edit, then publish automatically across all your platforms.",
    color: B_CYAN,
  },
];
...
// ─── Scene 4 — Social proof stats (480–600f, 4s) ─────────────────────────────
const STATS = [
    ...
    <AbsoluteFill style={{ background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: fadeIn * fadeOut }}>
      ...
      <div style={{ fontFamily: "'Segoe UI', Arial, sans-serif", width: "100%", padding: "0 120px" }}>
        ...
        <div style={{ display: "flex", gap: 60, alignItems: "center" }}>
          {STATS.map((s, i) => (
            <StatCard key={i} value={s.value} label={s.label} delay={20 + i * 20} color={colors[i]} />
          ))}
        </div>

        <GradientBar style={{ width: "100%", marginTop: 60, opacity: interpolate(frame, [60, 80], [0, 1], { extrapolateRight: "clamp" }) }} />
      </div>
    </AbsoluteFill>
  );
}

// ─── Scene 5 — CTA (600–750f, 5s) ────────────────────────────────────────────
function SceneCTA() {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
export const ProductLaunch = () => {
  return (
    <AbsoluteFill style={{ background: BG }}>
      ...
      {/* Scene 2: Reveal     90 – 210 (4s)  */}
      <Sequence from={90}  durationInFrames={120}> <SceneReveal />  </Sequence>
      {/* Scene 3: Features   210– 480 (9s)  */}
      <Sequence from={210} durationInFrames={270}> <SceneFeatures /></Sequence>
      {/* Scene 4: Stats      480– 600 (4s)  */}
      <Sequence from={480} durationInFrames={120}> <SceneStats />   </Sequence>
      {/* Scene 5: CTA        600– 750 (5s)  */}
